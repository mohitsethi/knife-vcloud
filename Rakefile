...
GEM_NAME = "knife-vcloud"

spec = eval(File.read("knife-vcloud.gemspec"))
...
task :install => :package do
  sh %{gem install pkg/#{GEM_NAME}-#{KnifeVCloud::VERSION} --no-rdoc --no-ri}
end

task :uninstall do
  sh %{gem uninstall #{GEM_NAME} -x -v #{KnifeVCloud::VERSION} }
end
